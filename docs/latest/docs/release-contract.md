...
Public `5.x` releases must classify compatibility explicitly.
...
Each `5.x` tagged release also includes at least one machine-readable
...
Release publication fails closed if a `5.x` tag is missing provenance evidence.
...
Each `5.x` manifest must publish a `release_integrity` object and each release
...
Each `5.x` release line must also make recovery answerable from shipped product
...
Public `5.x` capability negotiation is release-time negotiation, not request
header negotiation.
...
- capability ids published in a manifest are stable within the `5.x` line once
  released
...
The running `5.x` server also publishes discovery surfaces:
...
Every public `5.x` manifest must repeat these mappings in
`principal_contract`.
...
- published `error_code` values are stable within `5.x`
...
Every public `5.x` manifest must publish the scoped taxonomy in
...
Every public `5.x` manifest must publish these bindings in `act_contract`.
...
metadata and artifacts are not considered complete `5.x` publication until the
...
- For `5.x`, verify `releases/<tag>/integrity.json` and consume
